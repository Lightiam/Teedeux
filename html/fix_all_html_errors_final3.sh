...
mkdir -p backups_final_all3
find . -name "*.html" -exec cp {} backups_final_all3/ \;

echo "Fixing all remaining HTML syntax errors in all files..."

# Fix all anchor tags with href attributes
find . -name "*.html" -exec sed -i 's/<a href="\([^"]*\)">\([^<]*\)<\/a>/<button onclick="window.location.href=\x27\1\x27;" class="border-0 bg-transparent p-0">\2<\/button>/g' {} \;
...
find . -name "*.html" -exec sed -i 's/<a class="\([^"]*\)" href="\([^"]*\)">\([^<]*\)<\/a>/<button class="\1" onclick="window.location.href=\x27\2\x27;" class="border-0 bg-transparent p-0">\3<\/button>/g' {} \;
...
find . -name "*.html" -exec sed -i 's/<a class="\([^"]*\)" \([^>]*\) href="\([^"]*\)">\([^<]*\)<\/a>/<button class="\1" \2 onclick="window.location.href=\x27\3\x27;" class="border-0 bg-transparent p-0">\4<\/button>/g' {} \;
...
find . -name "*.html" -exec sed -i 's/<a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown"/<button class="nav-link dropdown-toggle border-0 bg-transparent" data-bs-toggle="dropdown"/g' {} \;
